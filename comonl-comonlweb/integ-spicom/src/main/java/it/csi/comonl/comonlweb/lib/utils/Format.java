...
public class Format {
    ...
    public static final String DEFAULT_DATA_FORMAT ="dd/MM/yyyy";
    
    public static final String SESSO_F = "F";
    public static final String SESSO_M = "M";
    
    public static final String ZERO_HHMM_MENSILE = "00:00";
  ...
  public static int parseIntDefaultZero(String value) {
      int n = 0;
      try {
          n = Integer.parseInt(value);
      }
  ...
  public static String parseIntZero(int i) {
      try {
          if(i == 0) {
              return "";
          }
  ...
  public static String toStrInt(BigDecimal bd) {
...
//  }
  ...
  public static int confrontaDate(String d1, String d2) {
      
      Date data1 = convertiStringaInData(d1);
      Date data2 = convertiStringaInData(d2);
...
// confronto particolare fra due date: oltre all'anno, mese, giorno, anche ore, minuti e secondi
  ...
  public static boolean confrontaData1MaggioreUgualeData2(GregorianCalendar data1, GregorianCalendar data2) {
      if(data2.before(data1) || data2.equals(data1)) {
          return true;
      }
      return false;
  }

  public static boolean confrontaData1MaggioreUgualeData2(Date data1, Date data2) {
      if(data2.before(data1) || data2.equals(data1)) {
          return true;
      }
      return false;
  ...
  public static boolean confrontaData1MaggioreData2(Date data1, Date data2) {
  ...
  public static boolean verificaDataConAnnoMinoreAnnoPrecedente(String data) {
      String annoCorrente = getAnnoCorrenteStringa();
      int annoPrec = parseInt(annoCorrente) - 1;
  ...
  public static boolean verificaDataConAnnoMaggioreUgualeAnnoPrecedenteENonSuperioreSysdate(String data) {
      String annoCorrente = getAnnoCorrenteStringa();
      int annoPrec = parseInt(annoCorrente) - 1;
      
      String anno = data.substring(6);
      int annotoCheck = parseInt(anno);
      
      Date dataToCheck = parseDate(data);
      
      if (annotoCheck >= annoPrec && confrontaData1MaggioreUgualeData2(new Date(), dataToCheck)) {
          return true;
      }
  ...
  public static int getAnnoCorrenteIntero() {
      try {
          String sysdate = convertiDataInStringa(new Date());
          String annoCorrente = sysdate.substring(6);
          return parseInt(annoCorrente);
      }
      catch(Exception e) {
          return 0;
      }
...
//    String actualMonthMax = "" + data.getActualMaximum(Calendar.DAY_OF_MONTH);
  ...
  public static Date creaJavaUtilDate(String data) {
      Date ris;
        SimpleDateFormat formatter = new SimpleDateFormat("dd/MM/yyyy", java.util.Locale.ITALY);
        try {
  ...
  public static String convertiDataEOraInStringaFormato_ddMMyyHHmmss(java.util.Date data) {
      if (data == null)
          return null;
      SimpleDateFormat formatter = new SimpleDateFormat("ddMMyyHHmmss", java.util.Locale.ITALY);
      return formatter.format(data);
  }
...
//  }
   ...
   *   restituisce la DIFFERENZA fra due date in giorni
   */
//  Non funziona se nel periodo considerato rientrano date  di passaggio legale-solare solare-legale  
//  public static int dateDiffDays(java.util.Date d1, java.util.Date d2) {
//    int result;
//    if (d2.after(d1))
//      result = (int) ( (d2.getTime() - d1.getTime()) / 86400000);
//    else
//      result = (int) ( (d1.getTime() - d2.getTime()) / 86400000);
//    return result;
//  }
  ...
  public static int dateDiffDays(java.util.Date d1, java.util.Date d2) {
        ...
        double data1 = d1.getTime();
        double data2 = d2.getTime();
        
        int result;
        if (d2.after(d1))
          result = (int) Math.round((data2 - data1) / 86400000);
        else
          result = (int) Math.round((data1 - data2) / 86400000);
        return result;
      }
...
//    cal.setTime(date);
//    
//    // se siamo al 31/12, somma un anno
//    Conversioni util = Conversioni.getInstance();
//    if(util.checkGiornoFineAnno(cal)) {
...
//  }
  ...
  // aggiunge un giorno a una data di tipo GregorianCalendar, e restituisce la data
  public static Date dateAddDay(GregorianCalendar cal) {
//    GregorianCalendar cal = new GregorianCalendar();
//    cal.setTime(date);
      
      // se siamo al 31/12, somma un anno
//    Conversioni util = Conversioni.getInstance();
      if(checkGiornoFineAnno(cal)) {
          cal.roll(Calendar.YEAR, true);
      }
      // somma un giorno
      cal.roll(Calendar.DAY_OF_YEAR, true);
...
public static boolean convertiStringInBoolean(String value) throws Exception {
  if (value != null) {
    ...
    {
//        throw new Exception("BaseModel.convertiStringInBoolean: Valore di input:'" + value + "' non gestito. Valori ammessi:si/s/1/true no/n/0/false");
        throw new Exception("convertiStringInBoolean: Valore di input:'" + value + "' non gestito. Valori ammessi:si/s/1/true no/n/0/false");
    }
...
public static String convertiBooleanInSN(boolean value){
    return (value? "S":"N");
}
...
//  }
   ...
   * @return
   */
  private static Calendar getUltimoGiornoMese(int mese, int anno) {
...
//    int giorniDelMeseFinale = dataFinale.getActualMaximum(Calendar.DAY_OF_MONTH);
  ...
  public static int differenzaFraDueDate_inMesi(Date dataInizio , Date dataFine)
  {
        if(dataInizio == null || dataFine == null) return 0;
        
        GregorianCalendar gcDal = (GregorianCalendar)GregorianCalendar.getInstance();
        GregorianCalendar gcAl = (GregorianCalendar)GregorianCalendar.getInstance();
        
        gcDal.setTime(dataInizio);
        gcAl.setTime(dataFine);
        
        int annoDal = gcDal.get(GregorianCalendar.YEAR);
        int annoAl = gcAl.get(GregorianCalendar.YEAR);
        int meseDal = gcDal.get(GregorianCalendar.MONTH);
        int meseAl = gcAl.get(GregorianCalendar.MONTH);
               
        if(annoDal == annoAl){
            return meseAl - meseDal + 1;
        }else{
            return (annoAl - annoDal)*12 - meseDal + meseAl + 1;
        }
  }

  private static int numeroDiSettimaneIntereCompreseTraDueDate(Date d1, Date d2) {
      int ris = 0;
      int giorniDiDiff = dateDiffDays(d1, d2);
      
      ris = (giorniDiDiff - 1) / 7;
  ...
  public static String calendarToStringFormato_gg_mm_aaaa(Calendar c) {
      int giorno = c.get(Calendar.DATE);
      //System.out.println("..... giorno = " + giorno);
      // il mese, partendo da gennaio (valore 0), deve essere incrementato di 1
...
//  }
  ...
  static public int dateDiffDaysConSegno(Date dataFine, Date dataInizio) {
    ...
    double dataEnd = dataFine.getTime();
    double dataStart = dataInizio.getTime();
    
    result = (int) Math.round((dataEnd - dataStart) / 86400000);
...
//  }
  ...
  /*
    ...
    if(limiteContratto.before(fine)){
      ...
      String dataInizio = "04/01/2010";
      String dataFine = "04/04/2010";
      
//    Date d1 = DateUtils.convertiStringaInDataDDMMYYY(dataInizio);
//    Date d2 = DateUtils.convertiStringaInDataDDMMYYY(dataFine);
      
      //System.out.println("diff: " + dateDiffDays(d1, d2));
      
      //System.out.println("diff: " + dateDiffDaysConSegno(d1, d2));
      
      
      
      ////System.out.println("inferiore?: " + (dateDiffDays(d1, d2)< 90));
...
//    //System.out.println(" *************  numero MESI : " + result + " ************** ");
  ...
  public static Long convertiBigDecimalInLong(BigDecimal bd) {
      try {
//      BigDecimal bd = new BigDecimal("2000");
          int bdInt = bd.intValue();
    
          Long l = new Long(String.valueOf(bdInt));
...
//    }
    ...
    public static GregorianCalendar stringToDate(String inputString)
    {
      ...
      try
      {
//        SimpleDateFormat myFormat = (SimpleDateFormat) DateFormat.getDateInstance();
          SimpleDateFormat sdf = (SimpleDateFormat) DateFormat.getDateInstance();
        ret.setTime(sdf.parse(inputString));
      }
      catch(Exception e)
      {
        ret = null;
    ...
    public static String dateToString(GregorianCalendar inputDate)
    {
      if(inputDate != null)
        return SimpleDateFormat.getInstance().format(inputDate.getTime());
      else return null;
    }

    public static String dateToStringConOraMinSec(GregorianCalendar inputDate)
    {
      if(inputDate != null) {
          SimpleDateFormat sdf = (SimpleDateFormat) DateFormat.getDateInstance();
          sdf.applyLocalizedPattern("dd/MM/yyyy HH:mm:ss");
          String data = sdf.format(inputDate.getTime());
          sdf.applyPattern("dd/MM/yyyy");
//        SimpleDateFormat.getInstance().applyLocalizedPattern("dd/MM/yyyy HH:mm:ss");
//          String data = SimpleDateFormat.getInstance().format(inputDate.getTime());
//          SimpleDateFormat.getInstance().applyPattern("dd/MM/yyyy");
        return data;
      }
    ...
    public static Integer stringToInt(String inputString){
      if (inputString == null) return null;
      Integer ret = null;
      try
      {
    ...
    public static GregorianCalendar convertDataToGregorian(java.sql.Date d){
    ...
    public static java.sql.Date convertGregorianToDate(GregorianCalendar gc){
      if(gc==null) return null;
      java.sql.Date d=new java.sql.Date(gc.getTime().getTime());
      return d;
    }

    public static String fillCodiceFiscale (String codicefiscale){
...
//  }
    ...
    public static Date getCurrentDate() {       
        SimpleDateFormat fd =   new SimpleDateFormat("dd/MM/yyyy", java.util.Locale.ITALY);
        try {
            return fd.parse(Format.convertiDataInStringa(new Date()));
        } catch (ParseException e) {
            e.printStackTrace();
        }
        return null;
    ...
    public static ArrayList<String> convertiCollectionDiStringheInArrayList(Collection col) {
    ...
    // converte in minuti una stringa frmattata hh:mm
    ...
    public static String minutiToOreMinuti(Number minNum){
        if(minNum == null) return null;
        int min = minNum.intValue();
        int hh = min/60;
        int mm = min - (hh*60);
        String oreMin = (hh < 10 ? "0" + String.valueOf(hh) : String.valueOf(hh))  + ":" 
            + (mm < 10? "0" + String.valueOf(mm): String.valueOf(mm));
        return oreMin;
    }
    
    public static String getCodComuneDaCf(String codice){
        codice = codice.toUpperCase();
        return codice.substring(11, 15);
    }
    
    public static String getDataDaCf(String codice){
        codice = codice.toUpperCase();
        StringBuffer sb = new StringBuffer();
        String annoBlock = codice.substring(6, 8);
        char meseBlock = codice.charAt(8);
        int giornoBlock = Integer.parseInt(codice.substring(9, 11));
        
        if(giornoBlock > 40) {
            sb.append(String.format("%02d" ,giornoBlock - 40));
        } else {
            sb.append(String.format("%02d" ,giornoBlock));
        }
        sb.append("/");
        switch( meseBlock) {
            case 'A':  sb.append("01"); break;
            case 'B':  sb.append("02"); break;
            case 'C':  sb.append("03"); break;
            case 'D':  sb.append("04"); break;
            case 'E':  sb.append("05"); break;
            case 'H':  sb.append("06"); break;
            case 'L':  sb.append("07"); break;
            case 'M':  sb.append("08"); break;
            case 'P':  sb.append("09"); break;
            case 'R':  sb.append("10"); break;
            case 'S':  sb.append("11"); break;
            case 'T':  sb.append("12"); break;              
        }
        sb.append("/");

        if(Integer.parseInt(annoBlock) <= 10) {
            sb.append("20"+annoBlock);
        } else {
            sb.append("19"+annoBlock);
        }
        
        return sb.toString();
     }
    
    public static String getSessoDaCf(String codice){
        int giornoBlock = Integer.parseInt(codice.substring(9, 11));
        return (giornoBlock > 40? SESSO_F : SESSO_M);
    }
 ...
 // metodo per calcolare la data di pasqua avuto in ingresso un anno
    ...
    public static boolean isDataBetween(Date dateIn, Date dateEnd, Date dateValue){
        return !((dateEnd != null && dateEnd.before(dateValue)) ||
                 (dateIn != null && dateIn.after(dateValue)));
    }
